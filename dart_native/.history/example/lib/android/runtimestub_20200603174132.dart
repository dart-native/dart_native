...
class RuntimeStub extends JObject {
  ...
  String getString(String s) {
    Pointer<Pointer<Void>> pointers = allocate<Pointer<Void>>(count: 1);
    Pointer<Void> paramSizePointer = pointers[0];
    paramSizePointer.cast<Int32>().value = 2;


    Pointer<Void> paramPointer = generateParamBuffer(paramSizePointer);
    Pointer<Int8> temp = paramPointer.cast();
    Int8List list = temp.asTypedList(2);
    list[1] = "x".codeUnitAt(0);
    print("west1 ${ String.fromCharCode(list[1])}");


    pointers = allocate<Pointer<Void>>(count:  1);
    pointers.elementAt(0).cast<Int32>().value = 11;
    invokeJavaMethod(Utf8.toUtf8(""), Utf8.toUtf8(""), pointers);
